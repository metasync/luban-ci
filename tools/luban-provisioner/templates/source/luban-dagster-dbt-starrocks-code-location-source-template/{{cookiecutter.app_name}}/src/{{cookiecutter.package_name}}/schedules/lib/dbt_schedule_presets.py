...
def hourly_cron_schedule(
    *,
    name: str,
    cron: str,
    job_name: str,
    enabled: bool = True,
    partition_offset_hours: int = 0,
    partition_lookback_hours: int = 0,
    dedupe_across_ticks: bool = True,
):
    if partition_offset_hours < 0:
        raise ValueError("partition_offset_hours must be >= 0")
    if partition_lookback_hours < 0:
        raise ValueError("partition_lookback_hours must be >= 0")
