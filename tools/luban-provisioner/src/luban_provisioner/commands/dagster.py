...
@click.option(
    "--location-name", required=True, help="Name of the Code Location (usually the app name)"
)
@click.option("--location-host", required=True, help="Host/DNS of the Code Location Service")
@click.option(
    "--location-port", required=True, default=3000, help="Port of the Code Location Service"
)
...
def register_location(
    ...
    try:
        ...
        if not os.path.exists(overlay_dir):
            click.echo(
                f"Error: Environment overlay '{environment}' does not exist in platform repo.",
                err=True,
            )
            exit(1)
