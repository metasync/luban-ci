...
def get_git_provider(
    provider_name, token, server=None, organization=None, project=None, base_url=None
):
    """
    ...
    if provider_name == "github":
        ...
        if not server and not base_url:
            click.echo(
                "Error: git_server or git_base_url is required for Azure DevOps Server.", err=True
            )
            sys.exit(1)

        if server and (server == "dev.azure.com" or server.endswith(".visualstudio.com")):
            click.echo(
                "Error: git_provider=ado does not support Azure DevOps Services hosts (dev.azure.com / *.visualstudio.com). Use git_provider=azure.",
                err=True,
            )
            sys.exit(1)
