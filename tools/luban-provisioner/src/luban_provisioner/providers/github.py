...
class GitHubProvider(GitProvider):
    def __init__(self, token, organization, project=None, git_server="github.com"):
        ...
        self.api_url = (
            f"https://api.{git_server}"
            if git_server == "github.com"
            else f"https://{git_server}/api/v3"
        )
        self.headers = {
    ...
    def create_repo(self, name, description=None):
        ...
        if self.organization:
            click.echo(
                f"Attempting to create repo '{name}' in organization '{self.organization}'..."
            )
            resp = requests.post(
                f"{self.api_url}/orgs/{self.organization}/repos", headers=self.headers, json=payload
            )
            ...
            if resp.status_code == 404:
                click.echo(
                    f"Organization '{self.organization}' not found or not accessible. Checking user account..."
                )
            else:
                click.echo(
                    f"Failed to create in org '{self.organization}'. Status: {resp.status_code}, Body: {resp.text}",
                    err=True,
                )
        ...
        if self.organization and self.organization != current_user:
            click.echo(
                f"Target '{self.organization}' is not the authenticated user '{current_user}'. Aborting fallback.",
                err=True,
            )
            return None
        ...
        click.echo(
            f"Failed to create repo for user. Status: {resp.status_code}, Body: {resp.text}",
            err=True,
        )
        return None
    ...
    def create_webhook(self, repo_identifier, webhook_url, secret, events=["push"]):
        ...
        # Create new hook
        config = {"url": target_url, "content_type": "json", "secret": secret, "insecure_ssl": "0"}
    ...
    def set_default_branch(self, repo_identifier, branch_name):
        ...
        resp = requests.patch(
            f"{self.api_url}/repos/{owner}/{repo_name}", headers=self.headers, json=payload
        )
        if resp.status_code != 200:
            click.echo(
    ...
    def create_project(self, project_name, description=None):
        ...
        """
        click.echo(
            f"GitHub Provider: create_project is a no-op for '{project_name}'. Repositories are created directly under Organization/User."
        )
    ...
    def create_pull_request(
        ...
        payload = {"title": title, "body": description, "head": source_ref, "base": target_ref}
