...
- **Buildpack (python-uv)**: Fixed uv downloads by removing invalid `wget --netrc-file` usage; the buildpack now uses `curl --netrc-file` when available or `wget --netrc` (via `$HOME/.netrc`) for `uv-mirror-netrc`.
